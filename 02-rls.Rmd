...
En este caso, se tienen registros de la `Resistencia` y la `Edad` de `{r NROW(datos)}` soldaduras.

Con miras explorar si existe una relación entre estas dos variables, procedemos a construir un gráfico de dispersión en `R` de la siguiente manera:
...
A continuación procedemos a estimar el modelo de RLS para los datos de las soldaduras:

```{r}
## ajuste del modelo de RLS
(modelo <- lm(Resistencia ~ Edad, data = datos))
```
Los resultados muestran los valores de  $\widehat{\beta}_0$ y  $\widehat{\beta}_1$, por lo que $\widehat{\beta}_0 = 15.74$ y  $\widehat{\beta}_1 = -0.244$. Por lo tanto, el modelo de RLS ajustado está dado por 
...
Con estos resultados se concluye que $\hat{\sigma} = 0.591$. Por lo tanto, el modelo complejo ajustado puede expresarse como


\begin{align}
\widehat{\text{Resistencia}}_i &\sim N(\hat{\mu}_i, \hat{\sigma}^2),\\ \hat{\mu}_i &\sim 2627.82 - 37.15\,\text{Edad}, \\
\sigma^2 &= 0.591^2 = 0.349.
\end{align}
...
Finalmente, con una confianza del 95\%, $\sigma^2 \in(0.269, 0.472)$. 
...
En este caso,  `F-statistic` corresponde al estadístico de prueba calculado $F_\text{calculado}$, y los valores `1` y `98` a los grados de libertad del $MSR$ y $MSE$, respectivamente. Finalmente, el valor $p$ es $p=2.2\times10^{16}$, por lo que **rechazamos** $H_0$.
...
$$t_{\text{calculado}} = \frac{\hat{\beta}_1}{\widehat{\sigma}_{\widehat{\beta}_1}}$$
...
Para nuestro ejemplo, podemos obtener los estadísticos de prueba para $\beta_0$ y $\beta_1$ utilizando el objeto `modelo` que contiene los resultados del modelo de RLS:
